...
from config import positionSimConfig, actualPositionConfig
...
class Position:
    ...
    def plot(self, ax):
        ...
        ax.add_collection(
            PatchCollection(redRects, edgecolor="none", facecolor="red", alpha=positionSquareOpacity))


class Backtest:
    def __init__(self, klines: list, decisionMaker, maxOpenPositions=1, positionSize=100):
        self.klines = klines
...
╔═╣ BACKTEST RESULTS ╠═══════════════
║
║   Np / mdd                    {(self.stats["netProfit"] / self.stats["maxDrawdown"] * -1):.2f}
...
╚════════════════════════════════════
    ...
    def runBacktest(self):
        ...
        # bulk
        dataPoints = self.decisionMaker.extractDataPointsBulk(self.klines)
        ...
        for klineIndex in range(numOfKlines):
            ...
            print(f"\rProgress: {(klineIndex / (numOfKlines - 1)) * 100:.2f}% | cps: {(klineIndex + 1) / seconds:.2f} | {len(stats['totPositions'])} pos | {stats['netProfit']:.2f}€", end="", flush=True)

            # single (needs to be calculated every kline)
            # dataPoint = self.decisionMaker.extractDataPointSingle(self.klines[klineIndex])
            ...
            if len(openPositions) >= self.maxOpenPositions:
                ...
                # bulk
                dataPoint = dataPoints[klineIndex]

                direction = self.decisionMaker.getPositionDirection(dataPoint)
                ...
                if direction == 0:
                    ...
                    predictedPos = Position(
                        entryIndex=klineIndex + 1,  # +1 because it's a prediction for the future kline
                        exitIndex=None,  # we don't know
                        entryPrice=self.klines[klineIndex + 1]["open"],
                        direction=direction,
            ...
            if predictedPos is not None:
                ...
                # calculate tp and sl
                predictedPos.tpPrice = predictedPos.entryPrice + (
                            predictedPos.entryPrice / 100) * predictedPos.tp * predictedPos.direction
                predictedPos.slPrice = predictedPos.entryPrice - (
                            predictedPos.entryPrice / 100) * predictedPos.sl * predictedPos.direction
            ...
            for openPos in openPositions:
                ...
                # check sl
                if (openPos.direction == 1 and self.klines[klineIndex]["low"] < openPos.slPrice) or (
                        openPos.direction == -1 and self.klines[klineIndex]["high"] > openPos.slPrice):
                ...
                # check tp
                elif (openPos.direction == 1 and self.klines[klineIndex]["high"] > openPos.tpPrice) or (
                        openPos.direction == -1 and self.klines[klineIndex]["low"] < openPos.tpPrice):
                    openPos.exitIndex = klineIndex
